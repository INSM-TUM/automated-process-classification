...
use crate::dependency_types::{
    ...
    existential::DependencyType as ExistentialEnum,
...
impl std::fmt::Display for Classification {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Classification::SemiStructuredLooselyStructured => write!(f, "Semi-Structured / Loosely Structured"),
...
impl CalculatedPercentages {
    pub fn new(matrix: &InputMatrix) -> Result<Self, String> {
        ...
        for dependency_obj in matrix.values() {
            let temporal_type = dependency_obj.temporal_dependency.as_ref().map(|td| td.dependency_type);
            let existential_type = dependency_obj.existential_dependency.as_ref().map(|ed| ed.dependency_type);
            ...
            match temporal_type {
                None => { // No temporal dependency
                    match existential_type {
                        ...
                        Some(ExistentialEnum::NegatedEquivalence) => counts_none_negated_equivalence += 1,
                ...
                Some(TemporalEnum::Eventual) => {
                    if existential_type.is_some() { // Any existential with Eventual temporal
                ...
                Some(TemporalEnum::Direct) => {
                    if existential_type.is_some() { // Any existential with Direct temporal
                        counts_direct_any +=1;
                    }
...
fn calculate_by_most_indicators(
    ...
    let score_structured = (s1_indicators + s2_indicators) * 2 + bs1_indicators;

    let ss1_indicators = count_true_conditions(&primary_rule_check_results[2].1);
    let ss2_indicators = count_true_conditions(&primary_rule_check_results[3].1);
    ...
    let ls1_indicators = count_true_conditions(&primary_rule_check_results[4].1);
    let ls2_indicators = count_true_conditions(&primary_rule_check_results[5].1);
    ...
    if max_score == 0 {
        // If all scores are 0, no rule significantly matched by indicators
        // This might indicate a log that doesn't fit any category well, or all rules had 0 indicators.
        // Fallback to a default or error if no indicators fired for any category
    ...
    match top_categories.len() {
        ...
        2 => {
            ...
            if has_s && has_ss {
                ...
                // S and LS, but not SS
                // Defaulting to SemiStructured as an "in-between".
                // NOTE: this might indicate rule conflict or edge case.
                Classification::SemiStructured
            } else {
                 // Should not happen if top_categories.len() == 2 and elements are distinct
                Classification::Error("Unexpected combination in top categories (2).".to_string())
            }
        }
        3 => Classification::SemiStructured, // All categories tied
        _ => Classification::Error("No category had a top score in most indicators (or internal error).".to_string()),
    }
}
...
pub fn classify_matrix(matrix: &InputMatrix) -> Classification {
    ...
    match primary_matched_categories_set.len() {
        ...
        _ => { // Multiple distinct primary categories matched
            ...
            if s_matched && ss_matched && !ls_matched {
                ...
                // Structured and Loosely Structured (but not Semi-Structured)
                // Fall through to "most indicators"
            } else if s_matched && ss_matched && ls_matched {
                // All three matched, fall through to most indicators
            }
        }
    }
    ...
    match secondary_matched_categories_set.len() {
        0 => { /* Proceed to Step 5 (Most Indicators) */ }
        1 => {
         ...
         _ => { /* Multiple secondary categories matched, or 0. Proceed to most indicators. */ }
    }

    // Step 5: Calculate by most indicators
    // This is the final fallback if previous steps are inconclusive.
...
mod tests {
    ...
    fn dd(
        ...
        DetailedDependency::new(
            from.to_string(),
            to.to_string(),
            temporal.map(|(t_type, t_dir)| {
                TemporalDependency::new(from, to, t_type, t_dir)
            }),
            existential.map(|(e_type, e_dir)| {
                ExistentialDependency::new(from, to, e_type, e_dir)
            }),
        )
    }

    // Simpler aliases for enums used in tests
    fn t_dir() -> TemporalEnum { TemporalEnum::Direct }
    fn t_ev() -> TemporalEnum { TemporalEnum::Eventual }
    fn t_fwd() -> TemporalDirection { TemporalDirection::Forward }
    #[allow(dead_code)] fn t_bwd() -> TemporalDirection { TemporalDirection::Backward }

    fn e_imp() -> ExistentialEnum { ExistentialEnum::Implication }
    fn e_eq() -> ExistentialEnum { ExistentialEnum::Equivalence }
    fn e_neq() -> ExistentialEnum { ExistentialEnum::NegatedEquivalence }
    fn e_fwd() -> ExistentialDirection { ExistentialDirection::Forward }
    #[allow(dead_code)] fn e_bwd() -> ExistentialDirection { ExistentialDirection::Backward }
    fn e_both() -> ExistentialDirection { ExistentialDirection::Both }
    ...
    fn build_detailed_matrix_from_counts_array(counts: [usize; 10]) -> InputMatrix {
        ...
        add_entries(counts[5], dd("from", "to", Some((t_dir(), t_fwd())), Some((e_imp(), e_fwd())))); // (Direct FWD, Implication FWD)
        add_entries(counts[6], dd("from", "to", Some((t_dir(), t_fwd())), Some((e_eq(), e_both())))); // (Direct FWD, Equivalence BOTH)
        add_entries(counts[7], dd("from", "to", Some((t_ev(), t_fwd())), None)); // (Eventual FWD, None)
        add_entries(counts[8], dd("from", "to", Some((t_ev(), t_fwd())), Some((e_imp(), e_fwd())))); // (Eventual FWD, Implication FWD)
        add_entries(counts[9], dd("from", "to", Some((t_ev(), t_fwd())), Some((e_eq(), e_both())))); // (Eventual FWD, Equivalence BOTH)
        ...
        if total_provided_counts != 100 && total_provided_counts !=0 { // Allow 0 for empty test
             eprintln!(
    ...
    #[test]
    fn test_hybrid_h1_s2_and_ss2_match() {
        // S2 conditions: NN < 5%, NI <= 20%, EEq >= 10%, EI > 30%
        // SS2 conditions: NN < 25%, NI > 0.01%, EEq > 10%, EI < 40%
        //
        // To satisfy both S2 and SS2 parts:
        // NN: must be < 5% (e.g., 4%)
        // NI: must be > 0.01% and <= 20% (e.g., 18%)
        // EEq: must be >= 10% (e.g., 21%)
        // EI: must be > 30% and < 40% (e.g., 35%)
        // Sum: 4 + 18 + 21 + 35 = 78. Remainder = 22.
        let counts = [4, 18, 0, 0, 22, 0, 0, 0, 35, 21]; // NN=4, NI=18, DN=22, EI=35, EEq=21
        let matrix = build_detailed_matrix_from_counts_array(counts);
        assert_eq!(
            classify_matrix(&matrix),
            Classification::StructuredSemiStructured
    ...
    // FIXME: this log should be Structured, but it is classified as SemiStructured
    #[test]
    fn test_log05_structured() {
        // LS1: none_none > 0.20 (yes, 0%), none_implication < 0.35 (yes, 0%), eventual_equivalence < 0.10 (yes, 0%), eventual_implication < 0.30 (yes, 13%)
        // LS2: none_none > 0.50 (no), none_implication < 0.10 (yes), eventual_equivalence < 0.05 (yes), eventual_implication < 0.25 (yes)
        // NN=0, NI=0, NEq=0, NNEq=27, DN=53, DI=0, DEq=0, EN=7, EI=13, EEq=0
        // Percentages:
        // none_none: 0.0
        // none_implication: 0.0
        // none_equivalence: 0.0
        // eventual_equivalence: 0.0
        // eventual_implication: 0.13
        // none_negated_equivalence: 0.27
        // eventual_any_existential: 0.13 + 0.0 = 0.13 (EI + EEq with Eventual)
        // direct_any_existential: 0.0 (DI + DEq with Direct)
    ...
    // Second batch of synthetic logs
    #[test]
    ...
    #[test]
    fn test_log15_unstructured_by_u2() {
